...
class MaskEditor(object):
    ...
    def __init__(self, ax, X, mask, remove_golgi):
        self.ax = ax
        ...
        self.over_impose_mask()
        self.draw = False
        self.began_drawing = False
    ...
    def onclick(self, event):
        ...
        if self.draw:
            ...
            self.x = int(round(event.ydata))
            self.y = int(round(event.xdata))
            ...
            if event.button == 1: #left_mouse_button
                if self.began_drawing:
                    x_space = np.round(np.linspace(self.last_x,self.x)).astype(int)
                    y_space = np.round(np.linspace(self.last_y,self.y)).astype(int)
                    self.X[x_space,y_space, :,self.ind]=1
                    self.mask[x_space,y_space, self.ind]=1
                else:
                    self.X[self.x, self.y, :, self.ind] = 1
                    self.mask[self.x, self.y, self.ind] = 1
                ...
                self.X[self.x-self.er_size:self.x+self.er_size, self.y-self.er_size: self.y+self.er_size, :, self.ind] = 0
                self.mask[self.x-self.er_size:self.x+self.er_size, self.y-self.er_size: self.y+self.er_size, self.ind] = 0
    ...
    def over_impose_mask(self):
        white_mask = np.expand_dims(self.mask,axis=2)
        white_mask = np.concatenate((white_mask,white_mask,white_mask),axis=2)
        self.X[white_mask>0]=1


def slicer_editor(image, mask, new_mask_path):
    fig, ax = plt.subplots(1, 1)
    slicer = MaskEditor(ax, image, mask, remove_golgi=False)

    fig.canvas.mpl_connect('scroll_event', slicer.onscroll)
    fig.canvas.mpl_connect('key_press_event', slicer.onpresskey)
    fig.canvas.mpl_connect('motion_notify_event', slicer.onclick)
    plt.show()
    np.save(new_mask_path,slicer.mask.astype(float))
